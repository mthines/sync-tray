...
- Mount-specific: `config.mount_backend` (nfs/macfuse), `config.mount_at_startup` (true/false), `config.stream_cache_only` (true/false; `n/a` for non-mount — the union-overlay Cache-only mount toggle), `config.vfs_cache_mode`, `config.has_pinned_directories`, `config.pinned_directory_count`, `config.download_connections` (parallel `--transfers`/warm concurrency, 1–16; `0` for non-mount profiles), `config.cache_fs_type` (bucketed filesystem of the VFS cache volume: apfs/hfs/exfat/fat/ntfs/network/other/unknown; `n/a` for non-mount), `config.cache_volume` (internal/external/unknown; `n/a` for non-mount)
...
| `synctray.cli.invoked` | Counter | Headless `synctray` CLI invocations (`cli.command`: bounded verb — doctor/status/profiles/logs/test-remote/listremotes/sync/mount/unmount/reinstall/install/cache-move/profile-create/profile-show/profile-set/profile-enable/profile-disable/profile-delete/profile-list/help, or `(other)`; `cli.result`: ok/error). Never carries args, paths, profile names, or remotes. Recorded once per run by `runMeasured`, then flushed via `flushForExit` before the process exits |
...
- Mount read health: `Mount read health` after each probe (info when `healthy`, warn otherwise) with `mount.read_health`, `cache.fs_type`, `cache.volume`, `mount.backend`, `vfs.cache_files`, `probe.bytes`, `probe.duration_seconds`, `probe.first_byte_seconds`, `probe.throughput_mbps`, `probe.remote_bytes`; `Mount read health probe skipped` (warn) when no fully-cached ≥16 MB file exists (`no_candidate`) or the read failed. Probed from the 5-minute heartbeat, at most once per 30 min per mounted **streaming** profile, never during an offline warm, and only when telemetry is enabled (the probe does no I/O otherwise). Never a file name or path — the probed file is random and unrecorded.
